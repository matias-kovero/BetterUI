...
namespace BetterUI.Patches
{
  ...
  class EnemyHealt
  {
    ...
    public static class AlterDefaults
    {
      private static void Postfix(ref EnemyHud __instance)
      {
        __instance.m_maxShowDistance = Mathf.Min(Math.Abs(Main.maxShowDistance.Value), Main.maxDistanceAllowed);
      }
    }

    [HarmonyPatch(typeof(EnemyHud), "ShowHud")]
    public static class AddLvlToName
    {
      ...
      private static void Prefix(ref EnemyHud __instance, ref Character c)
      {
        ...
        else if(c.IsBoss())
        {
          // Edits to Boss HP Bar
          Text hpText = UnityEngine.Object.Instantiate(hudData.m_name, hudData.m_name.transform.parent);
          hpText.name = "BU_hpTextBoss";
          hpText.rectTransform.anchoredPosition = new Vector2(hpText.rectTransform.anchoredPosition.x, 0.0f); // orig.y = 21f
          hpText.text = $"<size=16>{hudData.m_character.GetHealth():0} / {hudData.m_character.GetMaxHealth():0}</size>";
          hpText.color = Color.white;
          UnityEngine.Object.Destroy(hpText.GetComponent<Outline>());
        }
        else
        {
          ...
          hudData.m_name.text = hudData.m_name.text.Insert(0, $"<size={Main.enemyHudTextSize.Value - 2}><color=white>Lv.{c.m_level} </color></size> ");
          Text hpText = UnityEngine.Object.Instantiate(hudData.m_name, hudData.m_name.transform.parent);
          hpText.name = "BU_hpText";
          hpText.rectTransform.anchoredPosition = new Vector2(hpText.rectTransform.anchoredPosition.x, 6.0f); // orig.y = 21f
          hpText.text = $"<size=14>{hudData.m_character.GetHealth():0}/{hudData.m_character.GetMaxHealth():0}</size>";
          hpText.color = Color.white;
          UnityEngine.Object.Destroy(hpText.GetComponent<Outline>());
          ...
          Vector2 biggerBar = new Vector2(hpRoot.sizeDelta.x, hpRoot.sizeDelta.y * 3f);
          hpRoot.sizeDelta = biggerBar;
          //UnityEngine.Object.Destroy(hudData.m_aware.gameObject);
          //UnityEngine.Object.Destroy(hudData.m_alerted.gameObject);
          hudData.m_alerted.gameObject.SetActive(false);
          hudData.m_aware.gameObject.SetActive(false);
          hudData.m_healthFast.m_bar.sizeDelta = new Vector2(hudData.m_healthFast.m_width, hpRoot.sizeDelta.y);
          hudData.m_healthSlow.m_bar.sizeDelta = new Vector2(hudData.m_healthSlow.m_width, hpRoot.sizeDelta.y);
          //(hudData.m_alerted.transform as RectTransform).anchoredPosition = new Vector2(0 - hpRoot.sizeDelta.x / 1.5f, 12.0f);
          //(hudData.m_aware.transform as RectTransform).anchoredPosition = new Vector2(0 - hpRoot.sizeDelta.x / 1.5f, 12.0f);
      ...
      private static void Postfix(ref EnemyHud __instance, Player player, float dt)
      {
        ...
        foreach(KeyValuePair<Character, EnemyHud.HudData> keyValuePair in __instance.m_huds)
        {
          ...
          else
          {
            ...
            {
              Utils.FindChild(value.m_name.transform.parent, "BU_hpTextBoss").GetComponent<Text>().text = $"<size=16>{value.m_character.GetHealth():0} / {value.m_character.GetMaxHealth():0}</size>";
            }
            else
            {
              value.m_alerted.gameObject.SetActive(false);
              ...
              bool alerted = value.m_character.GetBaseAI().IsAlerted();
              value.m_name.color = (aware || alerted) ? (alerted ? Color.red : Color.yellow) : Color.white;
              Utils.FindChild(value.m_name.transform.parent, "BU_hpText").GetComponent<Text>().text = $"<size=10>{value.m_character.GetHealth():0}/{value.m_character.GetMaxHealth():0}</size>";
            }
          }
        }
        if (character != null)
